...
// Admin Seeder to ensure admin/Admin exists
$checkAdmin = $conn->query("SELECT user_id FROM users WHERE username = 'admin' AND role = 'Admin'");
if ($checkAdmin->num_rows == 0) {
    $hash = password_hash("Admin", PASSWORD_DEFAULT);
    $conn->query("INSERT INTO users (name, username, password, role, is_first_login) VALUES ('System Admin', 'admin', '$hash', 'Admin', 0)");
} else {
    $hash = password_hash("Admin", PASSWORD_DEFAULT);
    $conn->query("UPDATE users SET password = '$hash', is_first_login = 0 WHERE username = 'admin' AND role = 'Admin'");
}
