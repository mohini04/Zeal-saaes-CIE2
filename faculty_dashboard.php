...
// ----------------------------------------------------
// 1. DATABASE CONNECTION (Using config/db.php)
// ----------------------------------------------------
$pdo = require_once __DIR__ . '/config/db.php';

// Check if activities are empty and seed them
$stmt = $pdo->query("SELECT COUNT(*) FROM activities");
if ($stmt->fetchColumn() == 0) {
    $activities_data = [
        ['title' => 'Circuit Analysis & AC Fundamentals Quiz', 'type' => 'quiz', 'course' => 'BEE101 - Basic Electrical Engineering', 'subject' => 'BEE', 'unit' => 'Unit 1', 'batch' => '2025-29 (Sec A & B)', 'deadline' => '2026-07-28 23:59:00', 'total_marks' => 20, 'description' => 'Multiple choice quiz covering Kirchhoff\'s Laws, RLC Circuits, Phasor Diagrams, and 3-Phase Systems.'],
        ['title' => 'Nanomaterials & Polymer Tech Poster', 'type' => 'poster_making', 'course' => 'CH101 - Engineering Chemistry', 'subject' => 'Chemistry', 'unit' => 'Unit 2', 'batch' => '2025-29 (All Sections)', 'deadline' => '2026-07-30 18:00:00', 'total_marks' => 50, 'description' => 'Design an infographic poster on Industrial Water Treatment, Corrosion Control, or Synthetic Polymers.'],
        ['title' => 'Quantum Mechanics & Lasers PPT', 'type' => 'ppt', 'course' => 'PH101 - Engineering Physics', 'subject' => 'Physics', 'unit' => 'Unit 3', 'batch' => '2025-29 (Sec C & D)', 'deadline' => '2026-08-05 12:00:00', 'total_marks' => 30, 'description' => 'Prepare a 10-slide presentation on Fiber Optics Applications, Wave-Particle Duality, or Semiconductor Physics.'],
        ['title' => 'Differential Equations & Calculus Case Study', 'type' => 'case_study', 'course' => 'MA101 - Engineering Mathematics', 'subject' => 'Maths', 'unit' => 'Unit 4', 'batch' => '2025-29 (Sec E)', 'deadline' => '2026-08-02 23:59:00', 'total_marks' => 40, 'description' => 'Real-world application of Fourier Series and Laplace Transforms in signal processing and vibration analysis.']
    ];
    $stmt = $pdo->prepare("INSERT INTO activities (title, type, course, subject, unit, batch, deadline, total_marks, description) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
    foreach ($activities_data as $act) {
        $stmt->execute([$act['title'], $act['type'], $act['course'], $act['subject'], $act['unit'], $act['batch'], $act['deadline'], $act['total_marks'], $act['description']]);
    }
}

// Check if submissions are empty and seed them
$stmt = $pdo->query("SELECT COUNT(*) FROM activity_submissions");
if ($stmt->fetchColumn() == 0) {
    $activities = $pdo->query("SELECT id, deadline FROM activities")->fetchAll();
    $stmt = $pdo->prepare("INSERT INTO activity_submissions (activity_id, roll, name, submitted_at, file, status) VALUES (?, ?, ?, ?, ?, ?)");
    foreach ($activities as $act) {
        $deadline_time = strtotime($act['deadline']);
        $stmt->execute([$act['id'], '25CSE01', 'Aarav Sharma', date('Y-m-d H:i:s', $deadline_time - 86400), 'submission_doc_01.pdf', 'Submitted On Time']);
        $stmt->execute([$act['id'], '25CSE02', 'Bhavna Patel', date('Y-m-d H:i:s', $deadline_time - 3600), 'submission_doc_02.pdf', 'Submitted On Time']);
        $stmt->execute([$act['id'], '25CSE03', 'Chirag Verma', date('Y-m-d H:i:s', $deadline_time + 90000), 'submission_doc_03.pdf', 'Late Submission']);
        $stmt->execute([$act['id'], '25CSE04', 'Diya Singh', date('Y-m-d H:i:s', $deadline_time + 180000), 'submission_doc_04.pdf', 'Late Submission']);
    }
}
...
function get_type_meta($type) {
    ...
    return isset($type_map[$type]) ? $type_map[$type] : ['name' => ucfirst($type), 'icon' => 'fa-tasks', 'color' => '#3b82f6'];
}
...
function get_activity_by_id($id) {
    ...
    $res = $stmt->fetch();
    if ($res) {
        $meta = get_type_meta($res['type']);
        $res['type_name'] = $meta['name'];
        $res['icon'] = $meta['icon'];
...
function add_new_activity($data) {
    global $pdo;
    $stmt = $pdo->prepare("INSERT INTO activities (title, type, course, subject, unit, batch, deadline, total_marks, status, description) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'Active', ?)");
    $stmt->execute([
...
function update_activity($id, $data) {
    global $pdo;
    $stmt = $pdo->prepare("UPDATE activities SET title = ?, type = ?, course = ?, subject = ?, unit = ?, batch = ?, deadline = ?, total_marks = ?, description = ? WHERE id = ?");
    $stmt->execute([
        $data['title'], $data['type'], $data['course'], $data['subject'],
        isset($data['unit']) ? $data['unit'] : 'Unit 1', $data['batch'],
...
function recreate_activity($id) {
    ...
    if ($existing) {
        ...
        $new_data['deadline'] = date('Y-m-d H:i:s', strtotime('+7 days'));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'create_activity') {
        ...
        $submissions = $pdo->query("SELECT s.*, a.deadline, a.total_marks FROM activity_submissions s JOIN activities a ON s.activity_id = a.id")->fetchAll();
        $updateStmt = $pdo->prepare("UPDATE activity_submissions SET score = ?, status = ? WHERE id = ?");
        
        foreach ($submissions as $sub) {
            $deadline = strtotime($sub['deadline']);
            $max_marks = $sub['total_marks'];
            $submitted_at = strtotime($sub['submitted_at']);
            
            if ($submitted_at > $deadline) {
                $days_late = ceil(($submitted_at - $deadline) / 86400);
                $deduction_percentage = $days_late * $rate;
                if ($deduction_percentage > 100) $deduction_percentage = 100;
                $score = round($max_marks * (1 - ($deduction_percentage / 100)));
                $status = "Late Submission ($days_late Day" . ($days_late > 1 ? "s" : "") . ")";
            } else {
                $score = $max_marks;
                $status = "Submitted On Time";
            }
            $updateStmt->execute([$score, $status, $sub['id']]);
        }
            ...
            <div class="single-window-card">
                ...
                <div class="table-responsive">
                    <table class="custom-table">
                        ...
                        <tbody>
                            <?php 
                            $stmt = $pdo->prepare("SELECT * FROM activity_submissions WHERE activity_id = ? ORDER BY id ASC");
                            $stmt->execute([$id]);
                            $subs = $stmt->fetchAll();
                            if (empty($subs)): 
                            ?>
                            ...
                            <?php 
                                ...
                                foreach ($subs as $sub): 
                                    $is_late = strpos($sub['status'], 'Late') !== false;
                                    $status_color = $is_late ? '#f59e0b' : '#10b981';
                                    $icon = $is_late ? 'fa-clock' : 'fa-circle-check';
                            ?>
                            <tr>
                                <td><strong><?php echo htmlspecialchars($sub['roll']); ?></strong></td>
                                <td><?php echo htmlspecialchars($sub['name']); ?></td>
                                <td><span style="color: <?php echo $status_color; ?>; font-weight: 600;"><i class="fa-solid <?php echo $icon; ?>"></i> <?php echo htmlspecialchars($sub['status']); ?></span></td>
                                <td><a href="#" style="color: #38bdf8;"><i class="fa-solid fa-file-arrow-down"></i> <?php echo htmlspecialchars($sub['file']); ?></a><br><small style="color: var(--text-muted);"><?php echo date('M d, H:i', strtotime($sub['submitted_at'])); ?></small></td>
                                <td>
                                    <?php if ($sub['score'] !== null): ?>
                                    <strong style="color: <?php echo $status_color; ?>; font-size: 1.05rem;"><?php echo $sub['score']; ?> <span style="font-size: 0.85rem; color: var(--text-muted);">/ <?php echo $current_activity['total_marks']; ?></span></strong>
                                    <?php else: ?>
                                    <span style="color: var(--text-muted); font-size: 0.9rem;">Pending Review</span>
                                    <?php endif; ?>
                                </td>
                            </tr>
                            <?php 
